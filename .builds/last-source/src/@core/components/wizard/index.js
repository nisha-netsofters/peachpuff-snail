      ...
      return (
        <Fragment key={step.id}>
          ...
          <div
            ...
            data-target={`#${step.id}`}
          >
            <button type='button' className='step-trigger' style={{ color: "#105996 !imprtant" }}>
              <span className='bs-stepper-box' style={{ backgroundColor: index === activeIndex ? '#105996' : activeIndex > index ? '#10599630' : "", boxShadow: 'none', color: activeIndex > index && '#105996' }}>{step.icon ? step.icon : index + 1}</span>
              <span className='bs-stepper-label'>
                <span className='bs-stepper-title' style={{ color: index === activeIndex && "#105996" }}>{step.title}</span>
